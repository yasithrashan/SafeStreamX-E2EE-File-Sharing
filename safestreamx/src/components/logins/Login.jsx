...
const Login = () => {
  ...
  const authInProgressRef = useRef(false);
  const isMobileRef = useRef(false);
  const authStartTimeRef = useRef(0);
  const navigationPendingRef = useRef(false);
  const authProcessedRef = useRef(false);

  // Detect mobile devices and setup auth state listener
  useEffect(() => {
    ...
    const urlParams = new URLSearchParams(window.location.search);
    if (urlParams.has('authRedirect')) {
      console.log("Detected auth redirect return");
      authStartTimeRef.current = parseInt(urlParams.get('authStart') || Date.now());
      authInProgressRef.current = true;
      setIsAuthenticating(true);
      setAuthMessage("Completing authentication...");
    }

    // Auth state listener for continuous monitoring
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (authProcessedRef.current) return;
      
      if (user && authInProgressRef.current && !navigationPendingRef.current) {
        console.log("Auth state changed - authenticated user detected");
    ...
    const handleRedirectResult = async () => {
      try {
        ...
        if (result?.user) {
          ...
          // If we have auth params but no result, check current user
          const currentUser = auth.currentUser;
          if (currentUser) {
            await processUserLogin(currentUser);
          } else {
            // Final fallback after timeout
            setTimeout(() => {
              if (!authProcessedRef.current && authInProgressRef.current) {
                console.log("Auth flow timeout - resetting");
                resetAuthState();
  ...
  const resetAuthState = () => {
    ...
    setIsAuthenticating(false);
    setLoading(false);
    // Clear any auth-related URL params
    window.history.replaceState({}, document.title, window.location.pathname);
  };
  ...
  const handleAuthError = (error) => {
    ...
    switch (error.code) {
      ...
      case 'auth/web-storage-unsupported':
        errorMessage = "Your browser settings may be blocking authentication.";
        break;
  ...
  // Process user login and navigation
  const processUserLogin = async (user) => {
    try {
      if (!user || navigationPendingRef.current) return;
  ...
  const handleGoogleSignIn = async () => {
    try {
      ...
      if (isMobileRef.current) {
        console.log("Initiating mobile redirect flow");
        
        // Add parameters to track the auth flow
        const currentUrl = new URL(window.location.href);
        currentUrl.searchParams.set('authStart', Date.now().toString());
        currentUrl.searchParams.set('authRedirect', 'true');
        window.history.replaceState({}, document.title, currentUrl.toString());
        
        // iOS needs a slightly longer delay
        ...
        // Desktop uses popup flow with redirect fallback
        try {
          const result = await signInWithPopup(auth, provider);
          await processUserLogin(result.user);
        } catch (popupError) {
